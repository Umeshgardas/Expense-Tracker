...
export function Dashboard() {
  ...
  const [actionStatus, setActionStatus] = useState<'idle' | 'adding' | 'updating'>('idle');
  ...
  const fetchMonthlyIncomes = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setMonthlyIncomes(data); // <-- Fixed typo here
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-background relative">
      ...
      <header className="border-b bg-card sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between gap-2">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold truncate">Expense Tracker</h1>
              <p className="text-muted-foreground text-xs sm:text-sm mt-1 truncate max-w-[180px] sm:max-w-none">{user?.email}</p>
            </div>
          ...
          <div className="lg:col-span-1 border rounded-lg bg-card overflow-hidden shadow-sm">
            ...
            {isFormOpen && (
              <div className="p-4 border-t bg-background animate-in fade-in slide-in-from-top-2 duration-150">
                <ExpenseForm onSubmit={handleAddExpense} isLoading={actionStatus === 'adding'} />
              </div>
            )}
          </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="w-full px-2.5 sm:px-4 py-3 flex items-center justify-between bg-muted/40 border-b flex-nowrap overflow-hidden gap-1.5 sm:gap-2">
                        ...
                        >
                          {!isFutureMonth && (isMonthOpen ? <ChevronDown className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" /> : <ChevronRight className="h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />)}
                          <span className="truncate">{m.name}</span>
                        </button>

                        {/* Financial Context Labels */}
                        {!isFutureMonth && (
                          <div className="flex items-center gap-1 sm:gap-3 text-[11px] sm:text-xs shrink-0 flex-nowrap">
                            {editingIncomeMonth === m.value ? (
                              <div className="flex items-center bg-background border rounded px-1.5 py-0.5 h-7 sm:h-auto">
                                <span className="text-muted-foreground font-mono mr-0.5">₹</span>
                                <input
                                  type="number"
                              ...
                              <div 
                                ...
                                className="flex items-center gap-0.5 sm:gap-1 cursor-pointer bg-background hover:bg-muted/80 border px-1.5 sm:px-2 py-1 rounded group transition-colors"
                              >
                                <span className="text-muted-foreground font-mono">Income:</span>
                                <span className="font-semibold text-emerald-600 font-mono">
                                  ₹<span className="inline sm:hidden">{baseIncome.toFixed(0)}</span>
                                  <span className="hidden sm:inline">{baseIncome}</span>
                                  {monthlyIncomes[m.value] !== undefined && <span className="text-[9px] text-primary ml-0.5 sm:ml-1 font-sans">(Set)</span>}
                                </span>
                            ...
                            <div className="flex items-center gap-0.5 sm:gap-1 bg-background border px-1.5 sm:px-2 py-1 rounded">
                              ...
                              <span className={`font-bold font-mono ${remainingMonthValue >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                ₹<span className="inline sm:hidden">{remainingMonthValue.toFixed(0)}</span>
                                <span className="hidden sm:inline">{remainingMonthValue.toFixed(2)}</span>
                              </span>
                            </div>
                          </div>
                        )}
                          ...
                          {Array.from({ length: daysInMonth }, (_, index) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                >
                                  <span className="flex items-center gap-0.5 font-medium text-muted-foreground shrink-0">
                                    {isDayOpen ? <ChevronDown className="h-3 w-3 shrink-0" /> : <ChevronRight className="h-3 w-3 shrink-0" />}
                                    Day {day}
                                  </span>
                                  <div className="flex items-center gap-1.5 sm:gap-3 min-w-0 justify-end flex-1 flex-nowrap overflow-hidden">
                                    {totalDayValue > 0 && (
                                      <span className="font-medium text-rose-500 font-mono text-[10px] sm:text-xs shrink-0">
                                        -₹<span className="inline sm:hidden">{totalDayValue.toFixed(0)}</span>
                                        <span className="hidden sm:inline">{totalDayValue.toFixed(2)}</span>
                                      </span>
                                    )}
                                    <div className="text-[10px] sm:text-[11px] font-mono border-l pl-1.5 flex gap-0.5 shrink-0 items-center">
                                      <span className="text-muted-foreground text-[9px] sm:text-[10px]">Bal:</span>
                                      <span className={currentDayRemaining >= 0 ? 'text-emerald-600 font-semibold' : 'text-rose-600 font-semibold'}>
                                        ₹<span className="inline sm:hidden">{currentDayRemaining.toFixed(0)}</span>
                                        <span className="hidden sm:inline">{currentDayRemaining.toFixed(2)}</span>
                                      </span>
                                    </div>
                                    <span className="text-[9px] font-mono bg-muted px-1 py-0.5 rounded text-muted-foreground shrink-0">
                                      {dayExpenses.length} <span className="hidden sm:inline">items</span>
                                    </span>
                                  </div>
                                </button>
                                    ...
                                    {activeInlineDay === dayKey ? (
                                      ...
                                      >
                                        ...
                                        <div className="flex gap-0.5 shrink-0">
                                          <Button type="submit" size="sm" className="h-7 text-[10px] sm:text-xs px-1.5 sm:px-2">
                                            Save
                                          </Button>
                                          <Button
                                            type="button"
      ...
      {actionStatus !== 'idle' && (
        <div className="fixed bottom-5 right-5 z-50 flex items-center gap-2.5 bg-neutral-900 text-neutral-50 dark:bg-neutral-50 dark:text-neutral-900 px-4 py-3 rounded-xl shadow-2xl border border-neutral-800 text-xs font-semibold tracking-wide shadow-black/20 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <span>
            {actionStatus === 'adding' ? 'Adding new transaction records...' : 'Updating changes to dashboard manifest...'}
          </span>
        </div>
      )}
    </div>
